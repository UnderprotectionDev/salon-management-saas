...
- **Avatars:** `react-nice-avatar@1.5.0` (client-side, no external API). Config stored as `avatarConfig: v.optional(v.any())` in `userProfile` table.
...
└── modules/         # Feature modules (domain-driven)
    ...
    ├── org-onboarding/  # Org creation wizard (3 steps: Basics, Contact, Hours)
    │   ├── components/  # LeftPanel, StepBasics, StepContact, StepHours, BottomNav, etc.
    │   ├── hooks/       # useOnboardingForm (orchestrator), useSlugAvailability, useLogoUpload, useSalonCreation
    │   └── lib/         # constants (STEPS, SALON_TYPE_OPTIONS, HOUR_PRESETS), utils (slugify, computeHoursSummary, DEFAULT_HOURS_SUMMARY), validators
    └── user-onboarding/ # User profile setup wizard (name, gender, avatar)
```
...
- **Avatars:** `react-nice-avatar` config stored as `avatarConfig` JSON in `userProfile`. Fallback: `genConfig(userId)` (deterministic). No server-side generation — all client-side. Helper: `src/modules/user-onboarding/lib/avatar.ts` → `generateAvatarConfig(gender?)`.

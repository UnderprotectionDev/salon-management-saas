...
function PhoneInput({
  ...
  maxInputLength,
  ...props
}: PhoneInputProps & { maxInputLength?: number }) {
...
function InputComponent({ className, ...props }: ComponentProps<typeof Input>) {
  const { variant, maxInputLength } = useContext(PhoneInputContext);

  const countDigits = (val: string | undefined) =>
    (val ?? "").replace(/\D/g, "").length;

  // Block digit keystrokes when the max digit count is reached.
  // onKeyDown fires BEFORE the library's onChange, so preventDefault()
  // stops the character from ever reaching the library's internal state.
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (maxInputLength != null) {
      const digits = countDigits(e.currentTarget.value);
      const isDigitKey = e.key >= "0" && e.key <= "9";
      const hasSelection =
        e.currentTarget.selectionStart !== e.currentTarget.selectionEnd;
      if (isDigitKey && digits >= maxInputLength && !hasSelection) {
        e.preventDefault();
        return;
      }
    }
  ...
  // Truncate pasted content so total digits don't exceed the max.
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    if (maxInputLength != null) {
      const currentDigits = countDigits(e.currentTarget.value);
      const pastedDigits = e.clipboardData.getData("text").replace(/\D/g, "");
      const allowed = maxInputLength - currentDigits;
      if (pastedDigits.length > allowed) {
        e.preventDefault();
        const truncated = pastedDigits.slice(0, Math.max(0, allowed));

...
export default function GeneralSettingsPage() {
  ...
  const form = useForm({
    defaultValues: {
      ...
      salonType: ([...(activeOrganization?.salonType ?? [])].sort()) as OrgSalonType[],
    },
            ...
            {(field) => (
              <div className="space-y-2">
                {SALON_TYPE_CATEGORIES.map((category) => {
                  ...
                  return (
                    <Collapsible key={category.key} defaultOpen={selectedInCategory.length > 0}>
                      <CollapsibleTrigger className="flex w-full items-center justify-between rounded-md border px-4 py-2.5 text-sm font-medium hover:bg-accent/50 transition-colors [&[data-state=open]>svg]:rotate-90">
                        <span className="flex items-center gap-2">
                          ...
                          <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
                            {selectedInCategory.length}/{category.types.length}
                          </span>
                        </span>
                        <svg
